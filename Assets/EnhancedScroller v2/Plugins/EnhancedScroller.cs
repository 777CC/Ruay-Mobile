...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        public void ImmediatelySnap(int index)
        {
            //Debug.Log();
            ScrollPosition = ((GetScrollPositionForDataIndex(index, CellViewPositionEnum.Before) + GetScrollPositionForDataIndex(index, CellViewPositionEnum.After)) / 2) - (_scrollRectTransform.rect.height / 2);
            //ScrollPosition = (GetScrollPositionForDataIndex(0, CellViewPositionEnum.Before) + GetScrollPositionForDataIndex(0, CellViewPositionEnum.After)) / 2 + (_scrollRect.preferredHeight / 2);
        }
        /// <summary>
        ...
        void Awake()
        {
            ...
            _scrollRect.gameObject.AddComponent<ScrollerSnap>();
        ...
        void OnEnable()
        {
            ...
            _scrollRect.GetComponent<ScrollerSnap>().onEndDrag = _ScrollRect_OnEndDrag;
        }
        ...
        private void _ScrollRect_OnValueChanged(Vector2 val)
        {
            ...
            {
                // if the speed has dropped below the threshhold velocity
                if (Mathf.Abs(LinearVelocity) > 0 && Mathf.Abs(LinearVelocity) <= snapVelocityThreshold)
                {
                    // Call the snap function
                    Snap();
                }
